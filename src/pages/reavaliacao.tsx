...
const Reavaliacao: FC<ReavaliacaoProps> = ({ idPessoa }) => {
  ...
  const [infoPessoa, setInfoPessoa] = React.useState<infoPessoa>();
  const [cpfPessoa, setCPFPessoa] = React.useState('');
  const [postgresJson, setPostgres] = React.useState();
    ...
    <Card sx={{ padding: '1.5rem', pb: '4rem' }}>
      ...
      >
        {(formikMeta) => {
          React.useEffect(() => {
            infoPessoa
              ? formikMeta.setFieldValue('nome', infoPessoa.nome)
              : null;
          }, [infoPessoa]);
